...
```
owl:Thing
Γö£ΓöÇΓöÇ Organisation
Γöé   ΓööΓöÇΓöÇ Company
Γö£ΓöÇΓöÇ Cluster
Γöé   Γö£ΓöÇΓöÇ Industrial_Alliance_Supply_Chain_Cluster
Γöé   Γöé   Γö£ΓöÇΓöÇ Materials_Cluster
Γöé   Γöé   Γö£ΓöÇΓöÇ Equipment_Cluster
Γöé   Γöé   Γö£ΓöÇΓöÇ Intellectual_Property_and_Electronic_Design_Automation_Cluster
Γöé   Γöé   Γö£ΓöÇΓöÇ Fabless_Cluster
Γöé   Γöé   Γö£ΓöÇΓöÇ Silicon_Foundry_and_Integrated_Device_Manufacturer_Frontend_Cluster
Γöé   Γöé   Γö£ΓöÇΓöÇ Outsourced_Semiconductor_Assembly_and_Test_and_Integrated_Device_Manufacturer_Backend_Cluster
Γöé   Γöé   Γö£ΓöÇΓöÇ Photonics_Cluster
Γöé   Γöé   Γö£ΓöÇΓöÇ Research_&_Development_and_Academia_Cluster
Γöé   Γöé   Γö£ΓöÇΓöÇ Associations_and_Others_Cluster
Γöé   Γöé   ΓööΓöÇΓöÇ Customer_Cluster
Γöé   Γöé       Γö£ΓöÇΓöÇ Applications_Cluster
Γöé   Γöé       Γö£ΓöÇΓöÇ Components_Cluster
Γöé   Γöé       ΓööΓöÇΓöÇ Electronic_Manufacturing_Services_and_Original_Design_Manufacturer_Cluster
Γö£ΓöÇΓöÇ Region
Γöé   Γö£ΓöÇΓöÇ USA
Γöé   Γö£ΓöÇΓöÇ China
Γöé   Γö£ΓöÇΓöÇ Taiwan
Γöé   Γö£ΓöÇΓöÇ South_Korea
Γöé   Γö£ΓöÇΓöÇ Japan
Γöé   ΓööΓöÇΓöÇ Other
ΓööΓöÇΓöÇ Report
    Γö£ΓöÇΓöÇ Market_Capitalization
    ΓööΓöÇΓöÇ Regional_Concentration
```
...
| Cluster | Description |
|---------|-------------|
| **Materials** | Ultra-high-purity substances and consumables used in fab and packaging ΓÇö silicon wafers, process gases, photoresists, CMP slurries, metals, dielectrics |
| **Equipment** | Capital tools for making, inspecting, assembling, and testing chips ΓÇö lithography scanners, deposition, etch, metrology, ATE systems |
| **IP / EDA** | Software and reusable IP blocks for semiconductor design, verification, and sign-off ΓÇö including CPU/GPU/connectivity IP for SoC integration |
| **Fabless** | Companies that design and sell chips while outsourcing wafer fabrication to foundries such as TSMC |
| **SiFo / IDM Frontend** | Silicon foundries and IDMs performing front-end wafer processing (100ΓÇô300 mm), including lithography, deposition, etch, implant, CMP |
| **OSAT / IDM Backend** | Assembly, packaging, and final test after wafer fab ΓÇö wafer bumping, die attach, advanced packaging, system-level test |
| **Photonics** | Integrated photonics and optical interconnects using silicon, silicon nitride, and IIIΓÇôV compound semiconductors for data centres, telecoms, and sensing |
...
| **Components** | Suppliers of passive and electro-mechanical building blocks ΓÇö capacitors, connectors, EMI filters, power components |
...
| `snapshotDate` | Market_Capitalization Γê¬ Regional_Concentration | `xsd:string` | Date of the observation snapshot in format `DD_MM_YYYY` |
...
Each `Market_Capitalization` individual represents a single **companyΓÇôclusterΓÇôdate** observation.

```
Market_Capitalization_<Company>_<DD_MM_YYYY>_<Cluster>
    Γö£ΓöÇΓöÇ :reportsOnCompany                       ΓåÆ :Company    (Object Property)
    Γö£ΓöÇΓöÇ :reportsOnCluster                       ΓåÆ :Cluster    (Object Property)
    Γö£ΓöÇΓöÇ :hasMarketCapitalizationValueBillionsUSD ΓåÆ xsd:decimal (Data Property)
    Γö£ΓöÇΓöÇ :hasProportionInCluster                 ΓåÆ xsd:decimal (Data Property)
    ΓööΓöÇΓöÇ :snapshotDate                           ΓåÆ xsd:string  (Data Property)
```
...
Each `Regional_Concentration` individual represents a single **companyΓÇôregionΓÇôdate** observation.

```
Regional_Concentration_<Company>_<DD_MM_YYYY>_<Region>
    Γö£ΓöÇΓöÇ :reportsOnCompany         ΓåÆ :Company    (Object Property)
    Γö£ΓöÇΓöÇ :reportsOnRegion          ΓåÆ :Region     (Object Property)
    Γö£ΓöÇΓöÇ :hasConcentrationInRegion ΓåÆ xsd:decimal (Data Property)
    ΓööΓöÇΓöÇ :snapshotDate             ΓåÆ xsd:string  (Data Property)
```
...
## ≡ƒôÉ Market Capitalisation Formulas

### 1. Market Capitalisation by Cluster

```
MC_j = ╬ú (MC_i ├ù P_ij)   for all companies i in Co
```
...
| `P_ij` | `hasProportionInCluster` ΓÇö proportion of company i's market cap allocated to cluster j |
...
```
MC_T = ╬ú MC_j   summed across all clusters j = 1 to n
```
...
```
MC_r = ╬ú (MC_i ├ù P_ir)   for all companies i in Co
```
...
| `MC_i` | Total market cap of company i (raw financial figure) |
| `P_ir` | `hasConcentrationInRegion` ΓÇö proportion of company i's market cap allocated to region r |
| `Co` | The set of all companies in the ontology |
...
```
MC_T = ╬ú MC_r   summed across all regions r = 1 to n
```
...
**Components:** Continental ┬╖ HELLA ┬╖ Murata ┬╖ TDK ┬╖ TE Connectivity ┬╖ Valeo ┬╖ Vishay ┬╖ W├╝rth Elektronik ┬╖ ZF ┬╖ Samsung ┬╖ Bosch
...
|--------|-------------|---------------------|
| 1965 ΓÇô 1994 | Annual (December 31) | `Instant_1965_12_31` ΓÇª `Instant_1994_12_31` |
| 1995 ΓÇô 2020 | Quarterly (Mar, Jun, Sep, Dec) | `Instant_1995_03_31` ΓÇª `Instant_2020_12_31` |
| 2021 ΓÇô 2025 | Monthly (end of month) | `Instant_2021_01_31` ΓÇª `Instant_2025_12_31` |
| 2026 | Weekly (from 2 January 2026) | `Instant_2026_01_02` ΓÇª `Instant_2026_05_01` |
...
The ontology models directional flows between clusters using three symmetric property pairs: `flowsGoodsTo` Γåö `recievesGoodsFrom`, `flowsMoneyTo` Γåö `recievesMoneyFrom`, and `flowsInformationTo` Γåö `recievesInformationFrom`.

```
Materials ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ SiFo/IDM Frontend
Equipment ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ SiFo/IDM Frontend
                               SiFo/IDM Frontend ΓöÇΓöÇΓû║ OSAT/IDM Backend
IP/EDA ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ Fabless
                               Fabless ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ SiFo/IDM Frontend
IP/EDA ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ Photonics
R&D/Academia ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ IP/EDA
OSAT/IDM Backend ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ Customer Cluster
Photonics ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ Customer Cluster
Customer Cluster ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû║ Applications Cluster
```
...
**Query 1 ΓÇö List all companies in the Fabless Cluster**
...
**Query 2 ΓÇö Get market cap value for all companies at a snapshot date**
...
**Query 3 ΓÇö Get all clusters a company belongs to**
...
**Query 4 ΓÇö Get all companies in a region via HQ**
...
**Query 5 ΓÇö Get all supply chain goods flows between clusters**
...
**Query 6 ΓÇö Get all regional concentration values for a company**
...
**Query 7 ΓÇö Get regional concentration for all companies at a specific snapshot**
...
| Tool / Resource | Role |
|-----------------|------|
| [Prot├⌐g├⌐](https://protege.stanford.edu/) | OWL 2 ontology authoring |
...
_Developed for the Industrial Alliance on Processors and Semiconductor Technologies (IAPST) ΓÇö Stress Test Working Group._
